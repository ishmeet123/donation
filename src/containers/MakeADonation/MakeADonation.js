...
export default class MakeADonation extends Component {
  ...
  render = () => (
    <>
      <p>To donate enter your Name and Amount and then press send buttom:</p>
      <input type='text' placeholder='enter name' onChange={event => this.setState({name: event.target.value})} />
      <br />
      <input type='text' placeholder='enter donation amount' onChange={event => this.setState({amount: event.target.value})} />
      <br />
